    ...
    with get_redis() as redis:
        ...
        for item in cache_data.items():
            ...
            if config['status'] != 'enabled':
                logger.info(f'微信媒体平台({config['appid']})已禁用，忽略刷新access_token')
                return
            try:
                client = wechat_manager.get_instance(config['appid'])
                client.fetch_access_token()
                logger.info(f'成功刷新微信媒体平台({config['appid']})的access_token')
            except Exception as e:
                logger.error(f'刷新微信媒体平台({config['appid']})的access_token失败：{e}')
