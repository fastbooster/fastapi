    ...
    try:
        with get_session() as db:
            if data['type'] == BalancType.TYPE_RECHARGE.value:
                result = db.query(BalanceModel).filter(BalanceModel.user_id == data['user_id'], BalanceModel.related_id == data['related_id']).first()
                if result:
                    logger.info(f'当前余额动账({task_id})已处理过, 本次忽略', extra=data)
                    return

            # 余额动账
            last = db.query(BalanceModel).filter(BalanceModel.user_id == data['user_id']).order_by(BalanceModel.id.desc()).first()
    ...
    try:
        with get_session() as db:
            if data['type'] == PointType.TYPE_RECHARGE.value:
                result = db.query(PointModel).filter(PointModel.user_id == data['user_id'], PointModel.related_id == data['related_id']).first()
                if result:
                    logger.info(f'当前积分动账({task_id})已处理过, 本次忽略', extra=data)
                    return

            # 积分动账
            last = db.query(PointModel).filter(PointModel.user_id == data['user_id']).order_by(PointModel.id.desc()).first()
            balance = last.balance if last is not None else 0
            data['balance'] = balance + data['amount']
