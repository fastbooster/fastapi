...
@router.post("/wechat/get_authorize_url", response_model=WechatOAuth2Response,
             summary="公众号登录第一步：获取微信授权登录地址（可选）")
def wechat_oauth(params: WechatOAuth2Form):
    """
    ...
    oauth_client = WeChatOAuth(**fields)
    return WechatOAuth2Response(authorize_url=oauth_client.authorize_url)
...
@router.post("/wechat/code2user", response_model=AuthSuccessResponse,
             summary="公众号登录第二步：通过 code 获取用户信息，自动登录（前后端分离, 推荐）")
...
def code2user(params: WechatOAuth2CallbackForm) -> dict:
    ...
    try:
        oauth_client = WeChatOAuth(**fields)
        access_token = oauth_client.fetch_access_token(params.code)
        user_info = oauth_client.get_user_info()
        return user.autoreg_from_wechatoauth2(
