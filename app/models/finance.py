...
class BalanceModel(Base):
    ...
    ix_user_id_type = Index(None, user_id, type)
    ix_user_id_created_at = Index(None, user_id, created_at)
...
class PaymentAccountModel(Base):
    ...
    account = Column(String(255), nullable=False, comment='账号', index=True)
    ...
    ix_user_id_type_status = Index(None, user_id, type, status)
...
class WithdrawModel(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True, comment='ID')
    user_id = Column(Integer, nullable=False, comment='用户ID')
    account_id = Column(Integer, nullable=False, comment='提现账号ID', index=True)
    ...
    audit_status = Column(SmallInteger, server_default='0',
                          comment='审核状态', index=True)
    audit_user_id = Column(Integer, server_default='0',
                           comment='审核人员ID', index=True)
    ...
    back_memo = Column(String(255), comment='后台备注')
    created_at = Column(TIMESTAMP, server_default=text(
        'CURRENT_TIMESTAMP'), comment='创建时间', index=True)
    updated_at = Column(TIMESTAMP, server_default=text(
        'CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'), comment='更新时间')

    ix_user_id_payment_status = Index(None, user_id, payment_status)
...
class ChenckinModel(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True, comment='ID')
    user_id = Column(Integer, nullable=False, comment='用户ID')
    type = Column(SmallInteger, server_default='1',
                  comment='类型')  # 类型 1:签到, 2:补签
    ...
    related_type = Column(String(100), comment='关联类型', index=True)
...
class BalanceRechargeModel(Base):
    ...
    trade_no = Column(String(32), nullable=False, comment='交易号', unique=True, index=True)
    ...
    refund_gift_amount = Column(
        DECIMAL(10, 2), server_default='0', comment='退款赠送数量')

    # 支付状态：0 创建成功/未支付（只有此种状态才能继续执行支付）1 支付成功, 2 支付失败, 3 交易关闭，4 退款中, 5 部分退款, 6 已退全款
    payment_status = Column(
        SmallInteger, server_default='0', comment='状态', index=True)
    payment_channel = Column(String(50), comment='支付渠道', index=True)
    payment_appid = Column(String(50), comment='支付APPID')
    payment_time = Column(TIMESTAMP, comment='支付时间')
    payment_response = Column(Text, comment='支付结果')
